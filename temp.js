        ...
        function filterNews() {
            ...
            const relevanceFilter = document.getElementById('news-filter-relevance').value;
        ...
        function renderNewsList(items) {
            ...
            items.slice(0, 100).forEach((n, idx) => {
                ...
                if (n.Sentiment_Score !== undefined) {
                    ...
                    let color = '#666';
                    if (label.includes('Pos')) color = 'var(--positive-color)'; // Green
                    else if (label.includes('Neg')) color = 'var(--negative-color)'; // Red
        ...
        function showNewsContent(item) {
            ...
            if (item.Content && item.Content.length > 0) {
                ...
                displayContent = item.Content.substring(0, limit) + '... <a href="' + item.URL + '" target="_blank" style="color:var(--primary-color); font-weight:bold; text-decoration: none;">Read More</a>';
            }
        ...
        function analyzeSelectedNews() {
            ...
            Object.keys(newsByTicker).forEach(ticker => {
                ...
                tickerNews.forEach((n, idx) => {
                    ...
                    newsText += `    ${n.Content.substring(0, 4000)}...\n\n`;
                });
                newsText += '\n';
            });
        ...
        function analyzeTickerChart() {
            ...
            if (relevantNews.length > 0) {
                newsText = relevantNews.map(n => `- ${n.Date}: ${n.Title} \n Content: ${n.Content}`).join('\n\n');
            }
        ...
        function analyzeGroup(group) {
            ...
            if (groupNews.length > 0) {
                newsText = groupNews.map(n => `- [${n.Date}] ${TICKER_NAMES[n.ticker]||n.ticker}: ${n.Title}\n Content: ${n.Content}`).join('\n\n');
            }
    ...
    callbacks: {
            ...
            groups.forEach(group => {
                ...
                group.tickers.forEach(t => {
                    ...
                    item.innerHTML = `
                        ...
                        <i class="material-icons" style="font-size:16px; color:red; cursor:pointer;" onclick="deleteTickerFromManage('${group.id}', '${t}', event)">delete</i>
                    `;
        ...
        function deleteTickerFromManage(groupId, ticker, event) {
            event.stopPropagation();
            event.preventDefault();
            const index = summaryTickersConfig[groupId].indexOf(ticker);
            if (index > -1) {
                summaryTickersConfig[groupId].splice(index, 1);
            }
            visibleTickers.delete(ticker);
